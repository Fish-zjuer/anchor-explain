...
function fetchSection(
   ...
   *         现在清单驱动的档位**只说照抄清单里那一行**、不提任何"相对谁算"的规矩
   *         （那正是当初把它引偏的东西）；`any` 档只说"写真实路径 / 自己查"。
   */
  const howToName =
    listMode === 'none'
      ? [
          ...
          // ⇒ 这次真的一个别的文件都取不到。
          // @anchor 这一段非有不可：上一版这种情况仍然教它"照抄清单里的名字"，
          //         而清单根本不存在 —— 模型的反应是**编一个名字**出来（用户实测的截图），
          //         然后被拒、再编、直到把轮数烧完。**提示词与闸门必须同一套事实**。
          '- **这次读不到锚点文件之外的任何文件**（没有可用的「可能相关的文件」清单）。',
          '  所以**不要请求别的文件**，也不要编造文件名 —— 就基于锚点处的原文作答；',
          '  某个宏/结构体到底是什么，用你的先验知识说明它的**作用**，不要假装看过它的定义。',
        ]
      : listMode === 'list'
        ? [
            '- **可以读锚点文件之外的相关文件** —— `path` **照抄**「可能相关的文件」清单里那一行的名字。',
            '  **那个清单就是这次能取的全部文件**：清单外的名字一律取不到，所以**不要自己拼路径**，',
            '  也**不要把清单里的名字改写成别的形状**（比如把 `Driver/transport/Inc/transport.h`',
            '  写成 `../Inc/transport.h`）—— 那样一定取不到，白费一轮。`path` 省略才是"锚点这个文件"。',
...
export function buildSystemPrompt(
  ...
  options: {
    ...
    sourceType?: 'code' | 'pdf';
    /**
     * `path` 该怎么写（S9a-fix10 / D123）：`'list'` = 照抄清单里的名字（`related` / `same-dir` 且有清单）；
...
export function buildUserPrompt(
  ...
  if (options.crossFile === true && candidates.length > 0) {
    parts.push(
      '## 可能相关的文件',
      // S9a-fix10（D119）：清单**就是**这次能取的全部文件；
      // S9a-fix12（D124）：**只给名字**（曾给过假名 `f1`，已去掉 —— 名字带信息，假名不带，
      // 而锚点的绝对路径本来就在上面「锚点」那一节里，假名一点也没换来隐私）。
      // @anchor 也不再说"按锚点文件所在目录算"那一套：上一版既教它一个基准、又给一个同形状的例子，
      //         模型照着把文件名换掉就得到一串不存在的路径（用户实测：6 次取件 5 次 ENOENT）。
      '这些文件可能和锚点文件逻辑相关（按相关性排序，`#include` 提到过的排最前）。',
      '**要读它们时，`path` 照抄下面某一行的名字**（一行一个文件）。清单**就是**这次能取的全部文件；',
      '清单之外的一律取不到 —— 所以**不要自己拼路径**，也**不要把名字改写成别的形状**。',
