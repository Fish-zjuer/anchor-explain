...
| 给**这一次**换取件范围 | 命令面板 → **Anchor: 选择这次的取件范围**（只影响本次会话，重载窗口还原） |

写得好的那些补充说明（"想重点讲什么"）**会被记住**：下次问的时候输入框已经填着上一次那句，
直接回车就用它，全选删掉就不用。

每次讲解用完的 token 会显示在**讲解面板最下面那一行**（输入 / 输出，以及缓存命中与未命中）——
只显示这一次，不保存。端点没返回用量时它会说"端点没有返回"，不会拿 0 糊过去。
