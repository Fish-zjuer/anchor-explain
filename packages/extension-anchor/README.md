...
**默认就开着**：AI 可以读**锚点文件之外**的相关文件 —— 宏在 `config.h`、结构体在 `ring_buffer.h`、
真正调用它的在别的 `.c` 里，只看你选中的那几行是讲不出"数据从哪来、给谁用"的。

| 值 | 允许读哪些文件 |
|---|---|
| `related`（默认） | **这次列给你的「可能相关的文件」清单**（见下）；**锚点不在工作区里时**（比如你是用「打开文件」打开的、或者窗口开在别的目录）清单的范围退化成**锚点所在的这一层**（锚点目录 + 上一层）—— 这样 `Src/` 旁边的 `Inc/xxx.h` 这类文件才进得来（**密钥、依赖、构建产物目录始终不进清单**） |
| `same-dir` | 只列锚点文件所在目录里的文件 |
| `off` | 只读锚点文件（S9a 之前的行为） |
| `any` | **不限范围**：写绝对路径就能读工作区之外的任何文件（跨仓库、共享 SDK），并额外给 AI 一个 `find_files` 工具让它自己查有哪些文件。密钥/依赖/构建产物**仍然不读** |

```jsonc
{ "anchorExplain.fetchScope": "off" }
```

**清单就是可取范围**（这一条很值得知道）：每次讲解前，扩展会在工作区里扫一遍代码文件，
按相关性排好后把**最多 `anchorExplain.maxCandidateFiles` 条**（默认 40）交给 AI ——
**AI 只能读清单里的那些文件**，清单外的一律拒。清单里每条左边是一个**假名**（`f1`、`f2`…），
AI 用假名点名要读谁；真实路径不出现在发给模型的 prompt 里。
自己的文件挤不进清单（CubeMX 之类工程里官方库会占掉不少名额）就把 `maxCandidateFiles` 调大，
或者用命令 **`Anchor: 选择这次的取件范围`** 把这一次改成「不限」。

**它是怎么被约束的**（不是"放开了随便读"）：

- 一次读**一个文件的一段**，单次上限由 `anchorExplain.maxFetchLines` 给（**默认 400 行**，
  上限 2000）；**要多了不会被拒，只会截到上限**（回灌的内容头部写着真实行范围），总次数仍受 `maxFetchRounds` 限制
- **只能引用它读过的文件**：讲解里出现没读过的文件路径，会被校验拦掉并让它重试
- 取件日志（输出面板「Anchor」，**以及讲解面板底部那块**）记着**每一轮读了哪个文件、哪几行、有没有被拒**
  —— 它是唯一能复核的地方。记的是**解析后的绝对路径**，所以"它到底读了哪个文件"没有含糊空间：
  `[21:58:31] 第 1 轮 取件 file {"start":10,"end":20,"path":"C:/…/test/fixtures/ring_buffer.h"} — 先看看文件头部有哪些定义（355 字）`
  面板底部那条会显示成人话：`第 1 轮 ring_buffer.h 10-20 行 接受 · 355 字`
- 相对路径按**锚点文件所在目录**算（写 `ring_buffer.h` 就对），也可以写工作区内的路径；
  讲解里的 `location` 无论写相对还是绝对，**都会被解析成同一个文件**（D67：这两处必须同一套坐标，
  否则"读过又引用"会被自己的闸门拦下）
- 每次讲解还会给模型一份**候选文件清单**（同目录、`#include` 提到过的排最前，封顶 40 条）——
  它不是许可，只是"可以问谁"的线索；清单扫不出来时输出面板会留一句，不会静默变成"没有相关文件"
- **讲到别的文件时编辑器会自动切过去**：同一个编辑器组、预览标签，不堆标签也不分屏；
  侧边栏里凡**不在锚点文件里**的位置，行号前面都带着文件名 —— 否则「第 16 行」看起来就像锚点文件的第 16 行

> **它为什么不自己到处翻**：读哪几个文件由**模型判断相关性**（提示词教它判据：宏、结构体、调用者），
> 我们只管边界。"没读过的文件不许出现在讲解里"这条是硬的 —— 想讲 `RB_CAPACITY` 是多少，就得真去读一次。

### 讲解风格（§6，S8 起可调）

**三档都是示范驱动**（D93），prompt 本体是用户给的模板（D94）：`# 角色` → `# 输出形状` →
`# 通用规则` → `# 档位规则`（只进当前档的一节）→ `# 取件`（工具循环必需）→ `# 示例（few-shot）`
（只进当前档的示范）。示范正文的编辑面是 `scripts/style-lab/exemplar/<档位名>.md`
（线上常量有同步锁，改 .md 后重新生成常量即可）。
...
> **先问一句：你是要"看一眼"，还是要"日常用"？**
> **日常用就直接 `pnpm link:ext`**（在仓库根）—— 它把扩展装进你平时的 VS Code，
> **重启后活动栏那个图标就一直在**，不用起开发宿主、不用 F5、不用敲命令。
> 改完代码 `pnpm build` + 「开发人员: 重新加载窗口」即可；`pnpm unlink:ext` 撤掉（D60）。
> 下面的方式 A/B 是"临时起一个干净环境"用的，调代码时更顺手（有断点、有调试控制台）。
...
| **PDF：讲解里说「无法确定这份文档的总页数 / 无法定位内容」** | 线1 读不到那份 PDF（拿不到页数 → 闸门不许按页取件）。真实原因在**输出面板「Anchor」**里 —— 那条拒绝现在会指过去（D74） | 看 `Anchor` 输出通道里那一行「取不到这份 PDF 的页数（…）」。若原因写的是 `Setting up fake worker failed`，那是**产物**的问题、不是你的 PDF 有问题：回仓库根 `pnpm build` 再重载窗口 |
