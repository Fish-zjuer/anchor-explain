...
## D118 v0.1.1 用补丁号发布：修复搭车，块流窗口如实标注为预览

**背景**：D117 修完之后用户说"**releases 更新一下。版本号改一下。**"

**决策**：

1. 两个扩展 `0.1.0` → **`0.1.1`**（补丁号）。
2. VSIX **从当前 master（`42e3535`）打**，也就是把与 D117 同一次提交进来的
   S-P1 / S-P2a（块流窗口 `anchorExplain.showBlocks`）**一起发出去** —— 不回退、不单独剥离。
3. Release 说明里**明写**这条命令是**预览性质、尚未经实际使用验证**，并说清
   "**它在包里**"与"**建议你现在用它**"是两件事。

**理由**：

- **为什么不回退 S-P2a**：它在 `commands.ts` / `package.json` 里与 D117 交织 ——
  同一张命令注册表、同一组 `activationEvents`。从 `42e3535` 里单独剥掉它，
  得到的就不是一个"能构建"的提交；而 D117 之前的回退点 `slice-S9a-fix8`
  **没有取件修复**。在"发一个带未验证新入口的包"与"发一个没有修复的包"之间，选前者。
- **为什么仍然用补丁号，不用 `0.2.0`**：号段对外的语义是"这一版**修了什么 / 破了什么**"，
  不是"这版**工程量**多大"。D117 是修；块流窗口是**纯新增入口**、不动既有链路，
  没有破坏性变更 —— `0.1.x` 是诚实的，`0.2.0` 反而在暗示升级有风险。
- **为什么必须写进说明**：包里有什么就说什么（同 D67 的立场）。用户没实操验证过 S-P2a；
  静默发出去 = 让第一个装它的人当小白鼠，而且**不知道自己在当**。
  写出来的成本是半屏字，不写的代价是信任。

**顺带钉住的**（这类文件最容易被漏）：`docs/DISTRIBUTION.md` 的版本号示例、
两个 `README.dist.md` 里的安装命令行 —— 后者会作为**包内 `README.md`** 进 VSIX，
写错版本号 = 拿到包的人照抄一条不存在的文件名。

**状态**：生效。两个 `.vsix` 四道校验全过（结构 / 必带 / 禁带 / 密钥扫描，
manifest `Version` 均 = `0.1.1`）；发布前 `pnpm check`：**467 单测全过**
（core 50 / pdf-blocks 58 / extension-anchor 333 / anchor-pdf 26），
四组冒烟 **388 条断言**（唯一 FAIL 是那条要 spawn 子进程的，本沙箱一律 `EBUSY`，
见 `SLICES.md` 的「S9a 落地结果」）。

## D119 清单即范围：候选清单改成"能取什么"的唯一定义，并给假名

**背景**：D118 把 v0.1.1 发出去之后，用户装了它、在自己的 CubeMX 工程上实测。
**范围那一层修好了**（第 1 轮 `../Inc/main.h` 取到 3035 字，上一版会被拒），
但**跨文件讲解仍然没成**：6 次取件只成 1 次，另外 5 次全是 `ENOENT`。
把候选清单按他那个工程真算一遍（`release/preview-candidates.mjs`）之后事情很清楚：
**它想要的那几个文件，清单里全都写着**（`../../App/Inc/esc.h` 第 1 条、
`../../Driver/transport/Inc/transport.h` 第 5 条、`../../Driver/dshot/Inc/dshot_dma.h` 第 3 条），
而它写出来的是 `../Inc/transport.h`、`../Src/esc.h` —— **在套 CubeMX 惯例**（先试 `../Inc/` 再试 `../Src/`），
不是在抄清单。

**根因**：不是范围太窄，而是"清单"与"能取的集合"**本来是两套东西** ——
清单是提示、闸门按 `roots` 判，两者交集之外的写法都能过闸门，所以模型一直猜。
提示词里那个例子（`../Inc/dshot_dma.h`）还恰好是个**能被套用的形状**：
它照着把文件名换掉，就得到了一串不存在的路径。

**决策**：

1. **清单就是可取范围，一条不差**：`ContextFetchPolicy.candidates` 与 prompt 里那份清单
   是**同一个数组**；它的过滤判据（`roots` + 黑名单）与闸门**同一份**——
   `isDeniedPath` 因此搬到 `fetchDeny.ts` 共用。`related` / `same-dir` 档下只有清单里的文件取得动。
2. **清单给假名**（`f1`、`f2`…），真实路径不进 prompt。三种写法都认，前提都是落回清单里某一条：
   假名（正路）／标签照抄／按锚点目录算的相对写法。**先确定性解析、再用后缀模糊兜底**；
   后缀命中多条时回一句"对上了多条，请写假名"（把选择权还给模型，不替它挑）。
3. **`any` 档不给清单**（整个文件系统列不完），改为给一个 `find_files` 工具让模型自己查 ——
   用户原话就是"any 给一个接口让它自己查"。反向同样重要：**清单驱动的档位不给这个工具**
   （给了它就会绕开清单）。
4. **`anchorExplain.maxCandidateFiles` 可配**（默认 40，1~400）——用户提的"或者可改范围数量"。
   它同时是"这次最多能读到几个别的文件"，所以下限 1（0 条等于跨文件全关，那是 `off` 档的语义）。
5. **拒绝文案改写**：说清**档位 + 这次有几个可选 + 正确写法（假名）+ 两个可调的设置**；
   清单为空时才报"允许的根"（那一刻它是"为什么一个都没有"的唯一线索）。

**没做，以及为什么**（诚实交代）：**没有解决"库文件挤占名额"**。实测那份工程 40 条里 25 条是
`Drivers/CMSIS/**`（候选共 344 条，其中官方库 320 个），`#include` 提到的那 6 条排序是对的，
坏在第 16 行往后按字母序填。`maxCandidateFiles` 是给用户的临时出路，
但"第三方树该不该降级"是个需要用户拍板的口味问题（判据一旦写死，就会在别人的工程里误伤），
所以这一条留着不动 —— 见 `SLICES.md` 的「下次第一件事」。

**状态**：生效。单测新增 30 条（清单过滤/假名/三种写法/多义/`find_files`/条数边界）；
链式冒烟新增与改写的断言见 `SLICES.md`。

## D120 用户自己给"这一次"定取件范围（只影响本次会话）

**决策**：新增命令 `Anchor: 选择这次的取件范围`（`anchorExplain.pickFetchScope`）。
四档 + 一条"用设置里的"，选中后**只覆盖本次会话**（内存里的 `scopeOverride`），重载窗口还原。

**理由**：四种范围合适的**场合**不一样（讲自己的模块用 `related`，读外部 SDK 用 `any`，
只想看一个文件用 `off`），而设置里那个值是给"平时"用的。让用户为了这一次去改一个全局设置、
再记得改回来，是把选择成本推给他 —— 而他一定会忘。每项都把**后果**写在标题里，
因为档位 id（`related` / `any`）本身不说明任何事：用户要判断的是"这次它能不能读到那个文件"。

**状态**：生效。状态行（`describeConfig`）报的是**生效值**，所以"我这次开的是哪一档"始终可查。

## D121 额外提示词会被记住并预填（做不到"按上箭头"，所以用预填）

**决策**：用户上一次写的那句额外提示词存进 `workspaceState`（**按工作区隔离**、不进 settings）。
下次问这句话时输入框**预填**它 —— 直接回车就是用它的原话，全选删掉就是不用它。

**理由**：用户的原话是"可能误操作、对上一次回答不满意，但可能给了很多的提示词，
没了，再写又烦又不能完全一样"。**他建议的"全空时按方向向上键自动填充"做不到**：
`showInputBox` 是工作台自己的控件，扩展**拿不到它的按键事件**（没有这个 API），
而注册一个全局"上箭头"键位会把编辑器里的光标移动一起吃掉 —— 那个代价不能接受。
能控制的是它的**初值**，那正好比按上键更省事（不用按任何键，内容已经在那儿）。
按 Esc 返回 `undefined` **不清记忆**（那多半是"这次不想说"，不是"把上次那句作废"）。

**状态**：生效。「对上一次回答不满意」那条另有出路：`Anchor: 重新讲一遍` 用同一个锚点重问，
锚点里带着原来的 `focus`，不必重打。

## D122 token 用量汇总：程序算、只显示、不保存

**决策**：`AssistantTurn.usage` 由 provider 从端点响应里读（**给什么记什么**），
编排层逐轮累计，通过 `deps.onUsage` 回调推给侧边栏，显示在**讲解面板最下面那一行**。
`cachedInput` / `uncachedInput` 尽量分开列。**不落盘、不进讲解历史、不进 `workspaceState`**
（用户原话："程序处理，不保存"）。

**理由与三条纪律**：

1. **拿不到就说拿不到**：端点没返回 `usage` 时那一行写"端点没有返回 token 用量"，
   **绝不画 0** —— 把"不知道"写成 0 是在编一个看起来很确定的数（D67 的同一条纪律）。
2. **缓存单独列**：命中与未命中的价钱差着倍数，只报一个总数等于把"贵在哪"掩掉。
   两种字段形状都认（DeepSeek 的 `prompt_cache_hit_tokens` / OpenAI 的
   `prompt_tokens_details.cached_tokens`），细项拿不到就不印那两栏，不硬凑。
3. **走回调而不是塞进 `ExplanationResult`**：那个类型是**冻结契约**，而这是展示信息；
   更实际的是它应当**边跑边更新** —— 讲解要跑几十秒，用户盯着面板时数字在涨，
   那是"它还在动"的一条额外证据。每次讲解开始时清零（同一个扩展实例会连着讲很多段，
   不清就会把上一段的账记到这一段头上，那种数字比不显示更糟）。

## D123 S9a-fix11：没打开文件夹时清单是空的（上一版的回归）+ 被拒的宽限轮数

**背景**：S9c 发出去几分钟后用户实测，贴回来的日志是这样的：

```
第 1 轮 Src/main.c 1-60 行 接受 · 2263 字
第 2 轮 f1 1-60 行 拒绝（"f1" 不在这次可取的清单里（当前取件范围 "related"）。
        这次**一个别的文件都取不到**（允许的根：…/Core/Src、…/Core）——「可能相关的文件」清单是空的…）
```

两件事，一件是**我做出来的回归**，一件事是他提的改进：

### ① 这是 S9c 的回归：范围里有文件，池子里没有

拒绝文案自己把矛盾写出来了：**允许的根有两个**（`…/Core/Src`、`…/Core`），**却"一个别的文件都取不到"**。

根因：他**只打开了一个文件**（没打开文件夹）→ `relatedRoots` 正确地退化成"锚点所在的这一层"
（锚点目录 + 上一层）→ 可池子来自 `vscode.workspace.findFiles`，**那个 API 只在工作区文件夹里找** →
没有文件夹 ⇒ 池子是空的 ⇒ 清单是空的 ⇒ **清单即范围** ⇒ 一个别的文件都读不到。

@anchor **比 D117 修之前还糟**：D117 之前至少锚点旁边那个文件是能读的（那正是 D117 修的东西）。
S9c 把"清单"提升成范围的唯一定义时，**忘了让清单的取材范围跟上范围的取材范围** ——
范围能算到工作区之外，池子却只扫工作区里面，于是两者在"没有工作区文件夹"这个常见形状上直接对立。

**决策**：`scanCodeFiles` 收下 `roots`，**工作区文件夹盖不住的那些根自己去走一遍目录树**
（`workspace.fs.readDirectory`，与 PDF 字节同一个立场：对 remote / 虚拟文件系统也成立）。
限制都是死的：最多 `SCAN_LIMIT` 个文件、最多 6 层、跳过黑名单目录、读不动就跳过那个目录。

**为什么不是"给空清单一条兜底路径"**：那等于在闸门里再开一条"没清单也能读"的路 ——
而"清单即范围"这条纪律的全部价值就在于**只有一条路**（D119 的教训：两套东西就是猜路径的温床）。

### ② 提示词与闸门必须同一套事实（`candidateMode` 加了 `'none'`）

同一个日志里还有一处：模型**编了一个 `f1`**。它没错 —— 系统提示词那时候还在教它
"`path` 写清单里第一列的假名"，而**清单根本不存在**（`buildUserPrompt` 只在清单非空时才写那一节）。
提示词只要与闸门说的不是同一件事，模型就会按提示词去试，而闸门一定拒它。

**决策**：`candidateMode` 从两种变成三种，由**档位与清单一起**决定：
`any` → `'path'`；清单非空 → `'alias'`；清单为空 → `'none'`。
`'none'` 那一段明说"这次读不到锚点文件之外的任何文件，不要请求别的文件，也不要编造文件名"。

### ③ 被拒之后额外给 2 轮

**用户的原话**："读到一个不允许的文件就死了，有点问题，建议给2次机会？" —— 诊断是对的。
被拒**不消耗取件预算**（规则 5 只管放行的那些），但它**消耗轮次**：
一次写错就把剩下的轮数挤掉，最后以 `MAX_ROUNDS_EXCEEDED` 收场，**用户什么都拿不到** ——
而那份讲解本来可以基于锚点原文给出来。"写错了 → 看回灌 → 改对"这条自纠路径
是我们自己设计的（§3.2「拒绝不抛错」），那就得让它在轮数上真的走得完。

**决策**：`turnLimit = maxFetchRounds + 2 + REJECTED_GRACE_TURNS(2)`。
上限仍然是**死的**（`maxFetchRounds + 4`），不会变成无限循环 —— 单测与冒烟都钉着这条。
为什么是 2：给一次改写法、再给一次"算了，就按现有信息作答"；再多边际收益很小，
而每一轮都是真金白银的模型调用。

**验收**：单测 +5（清单为空/非空的两种提示词口径、被拒 6 次后仍拿到讲解、
宽限是死的上限、`find_files` 在没文件夹时也能用），**365 例全过**；
链式冒烟 +3（**没有工作区文件夹时清单仍然非空 + 写 `f1` 能取到 + 日志里记的是解析后的绝对路径**，
并给桩补了 `workspace.fs.readDirectory` —— 桩不真的走目录树，这条路在冒烟里就永远走不到），
**212 条断言全过**。

**状态**：生效。随 v0.1.1 **第二次重新打包**发出。


## D124 S9a-fix12：去掉假名，清单只给名字（用户一句反问推翻了我的一条理由）

**背景**：S9c 的公告里写着"清单改给假名（`f1`、`f2`…）"。用户看了一句：
"**文件名也是有信息的，你改成 f1、f2 什么的是多此一举。**"

**先认错 —— 我给假名写的头号理由是错的。** 当时的名义是"真实路径不进 prompt"（隐私）。
但 `describeAnchor` **早就把锚点的绝对路径与所在目录写进 prompt 了**
（`文件路径：C:/.../Core/Src/main.c`、`所在目录：…\Core\Src`）。清单给不给假名，路径都在。
所以隐私这条理由从头到尾不成立。

**剩下的唯一好处**：假名不能被"改写"。D117 那版清单里写的是
`../../Driver/transport/Inc/transport.h`、也说了"照抄即可"，模型还是写成了
`../Inc/transport.h`（套 CubeMX 惯例）—— `f5` 没法被这样改写，因为它不是路径。

**但这条也不硬**：那次改写的压力有一半是提示词自己给的 —— 它**既教"写相对路径时按锚点文件所在目录算"，
又给一个同形状的例子**。模型照着套，是我们教出来的。S9c 已把那些教法全删，只留"照抄这一行"。
而且真正修好那个 bug 的是**"清单里的才算数"这条校验**（清单即范围），不是假名；
假名只是让"第一次就写对"的概率高一点。

**决策（用户拍板：去掉）**：

- 清单**只给名字**：相对工作区根的写法（锚点不在工作区里时退化成相对锚点目录），**它就是模型要照抄的那串**。
- 提示词只说"照抄某一行的名字"，并明说"不要自己拼路径、也不要把名字改写成别的形状"。
- 闸门认两种写法，前提都是**落回清单里某一条**：名字照抄（正路）、名字的唯一后缀（写短了不算白烧）。
  假名分支整个删掉（`findCandidate` 不再认 `f1`，写它会被拒）。
- `candidateMode` 的 `'alias'` 改名 `'list'`（机制变了，名字跟着变）。

**代价（用户已知情）**：模型自己改写名字的概率可能回升。但那种失败现在很便宜 ——
拒绝文案明说"照抄清单里那一行"，属自纠路径，被拒还有宽限轮数（D123）。

**收益**：prompt 少一层映射（一行一个名字，不再两列）；模型看见的就是真名字，
名字本身带的信息（目录结构、命名习惯）都还在 —— **这正是用户那条反问的分量所在**。

**状态**：生效。随 v0.1.1 第三次重新打包发出。
