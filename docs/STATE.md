...
**线2 的框选重测（D73 的验收，用户十秒钟就能判）**：

1. `pnpm build` 已经跑过（产物是新的）→ **重载窗口**（联接装法）
2. 命令面板 → `Anchor: 用 Anchor 打开 PDF` → 选一个 PDF
3. `Ctrl+Alt+S`：**光标要变成十字**（这一步过去就是死的）
4. 拖一个矩形 → 侧边栏应出讲解；**抬手后 PDF 上不留任何东西**；编辑器里一个框都不该有
5. **如果没有十字光标**：第一次按会说「页面还在加载」，再按一次会说「一直没有回应框选脚本」——
   那就把 Console 的报错发我（命令面板 → `Developer: Toggle Developer Tools` → Console）。
   **别只说"没反应"**，这一轮就是因为只有八个字，查得慢。

**然后是 S9a 的旧重跑**（跨文件；上次没过）：

1. 在 `test/fixtures` 里打开 `main.c`，选中第 40-48 行（`rb_pop`），`Ctrl+Shift+A`
...
2026-09-14，**S5/S6 补（D73）**：用户报「框选PDF，并没有反应」—— 查出来是线2 那条链路的
**第一颗螺丝**（`acquireVsCodeApi()` 一个 webview 只能成功调一次，而 pdf.js 先取走了），
一整片都没人知道，因为第一版把它 `catch` 成了静默。修法：注入脚本接管并共享实例 + 注入位置
提到上游脚本之前 + 两处"失败必须发声"。**线2 的注入脚本因此头一次有了行为夹具（9 条）**，
`pnpm check` 全绿：**289 测**（core 40 + ext 228 + pdf 21）/ 冒烟 73 + 146 + 74。

在此之前的 2026-09-13，**S8 收工**（固定按钮 + 开始界面），之后连着修了**三条"起宿主"的路**：
D58（就绪信号聚合）、D59（开发路径必须绝对）、**D60（`pnpm link:ext` 装成常驻扩展 ——
用户要的"像别的插件一样常驻"就是这条）**；
再往后是 **S9a 的一串实测返工**（D67 许可不可执行 / D68 假日志与重复讲解 / D69 落点画错 /
D70 整条路径印进标签 / D71 取件 60→400 行 / D72 讲完之后是死路 —— 按钮与键位两半）。
计划内的 F0/F1/F2 + S1~S7 也早已全部做完。
只剩**用户实操确认**（下面「下次第一件事」），没有待做的代码。
