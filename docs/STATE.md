...
**PDF 取件重测（D74 的验收，接在 D73 之后 —— 框选已经通了，现在看取件）**：

1. `pnpm build` 已经跑过（产物是新的）→ **重载窗口**
2. 「用 Anchor 打开 PDF」→ 框选一块 → `Ctrl+Alt+S` 那条链路照旧
3. **看侧边栏底部的取件日志**：那两行应该从「**拒绝**（无法确定这份文档的总页数…）」变成「**接受**」，
   并且讲解第一步应当引到那一页的**原文**（而不是说"我没拿到原文字"）
4. 若还是"拒绝"：**输出面板「Anchor」里那一行会写出原因**（「取不到这份 PDF 的页数（…）」）。
   把它发我 —— 现在这条路不会再静默了

**其余待确认（都是实操）**：

- 线2 的框选（D73）：`Ctrl+Alt+S` 光标变十字 → 拖一个矩形 → 侧边栏出讲解；抬手后 PDF 上不留东西
- 跨文件（S9a）：在 `test/fixtures` 里选中 `main.c` 的 `rb_pop`（40-48 行）按 `Ctrl+Shift+A`，
