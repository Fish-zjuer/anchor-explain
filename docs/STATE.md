...
用户报"讲解的部分布局不够鲜明，没有突出点，还有点没对齐"，已改完（D50）：

| 反馈 | 真因 | 处置 |
|---|---|---|
| 有点没对齐 | ① `上下文`(3 字)/`定义`(2 字) 标签宽度不同 → 讲解文字左边缘逐行错开；② `▸` 只出现在扫描行 → 那一行整体被顶右 | 标签列与标记槽都改成**固定宽**（`--anchor-tag-w` / `--anchor-gutter-w`），每行都占同样的槽 |
| 没有突出点 | 只压暗了**已讲过**的步骤，**还没讲的**和当前步一样亮 → 走到第 1 步时整屏都在喊 | `.step:not(.current) { opacity: 0.62 }`：之前之后一视同仁 |
| 布局不够鲜明 | 标题/摘要/正文/子行权重太接近 | 标题加大加粗、徽章缩小、当前步卡片加边框与底色、扫描行加蓝底 + 左侧强调线 + 加粗 |
| （自查发现） | 底部 sticky 工具条会盖住刚滚进来的扫描行 | 扫描行加 `scroll-margin-bottom: 64px` |

**新增 `pnpm preview:sidebar`**（D50）：本地排版预览。排版的取舍只能靠眼睛判，
而每次改完都让用户按 F5 看一眼代价太高 —— 现在改完先自己看一遍再交出去。
本轮靠它发现并修掉了三处（标签错开、扫描行顶右、以及预览脚本自己给 `body` 定死宽度
导致窄面板下"假溢出"）。
