...
> 第二次发出去之后用户又实测到 **S9c 带出来的一个回归**（只打开一个文件时清单为空），
> 于是有了 **S9c-fix11（D123）**。
> **两个 `.vsix` 与 Release 说明都按最新一次重新生成**，tag `v0.1.1` **被强制移到新提交**
...
> **这一版又被实测打回了一次，见下面的「S9c-fix11」。** 上面那份"完成"是自动化层面的。
...
## S9c-fix11 没打开文件夹时清单是空的（S9c 的回归）+ 被拒的宽限（D123）

**目标**：S9c 把"清单"提升成可取范围的唯一定义，却忘了让**清单的取材范围**跟上**范围的取材范围**。
用户上一版刚发出去就实测到了：范围里有文件（允许的根：`…/Core/Src`、`…/Core`），清单却是空的。

**范围**：

- `vscode/relatedFiles.ts`：`scanCodeFiles` 收 `roots`；工作区文件夹**盖不住**的那些根
  直接走目录树（`workspace.fs.readDirectory`，最多 6 层、跳过黑名单、读不动就跳过该目录）
- `commands.ts`：`buildFetchBoundary` 把 `roots` 传给扫描器
- `prompts/index.ts` + `en.ts`：`candidateMode` 加第三种 `'none'`（清单为空时明说"这次读不到别的文件"，
  **不再教假名**）
- `Orchestrator.ts`：`candidateModeOf`（档位 + 清单一起决定）；`turnLimit` 加 `REJECTED_GRACE_TURNS = 2`
- `scripts/smoke-walkthrough.mjs`：桩补 `workspace.fs.readDirectory`（桩不真的走目录树，
  这条路在冒烟里就永远走不到）
...
| 验收 | 靠什么 |
|---|---|
| **没有工作区文件夹时，清单仍然非空** | 链式冒烟：`workspaceFoldersValue = []` → prompt 里出现 `- \`f1\`` |
| 假名能取到文件，且日志记的是解析后的绝对路径 | 链式冒烟：写 `f1` → 不被拒、内容头部有真实行范围；输出面板那行里有 `ring_buffer.h` |
| 清单为空时提示词不再教假名 | 单测两条（空 / 非空各一条，口径按事实切换） |
| 被拒之后仍能拿到讲解（不会"就死了"） | 单测：6 次被拒 + 1 次作答 = 7 轮，仍然出讲解 |
| 宽限是**死的上限** | 单测 + 冒烟（`fetchCalls.length` 有界、`MAX_ROUNDS_EXCEEDED` 照样抛） |

**回退点**：`slice-S9a-fix10`（S9c 之前）。三处都是本地的：`scanCodeFiles` 的 `roots` 参数、
`candidateMode` 的第三种取值、`turnLimit` 的 `+2`。

**状态**：**完成**。

### S9c-fix11 落地结果（2026-09-22）

- **单测**：extension-anchor **360 → 365**（+5），四个包 494 → **499 例全过**。
- **冒烟**：链式 **209 → 212** 条断言（+3），四组共 **393 条**；唯一 FAIL 仍是那条 `spawn EBUSY`。
- **typecheck / build**：全过。
- **顺带修正的数字**：`turnLimit` 变了之后，"一直要上下文"那条冒烟的期望从「被拒 2 次」改成
  「被拒 4 次」（turnLimit = 1 + 2 + 2 = 5，第一次放行、后 4 轮全被拒），
  并在注释里写清这个数字是**按公式推出来的**，不是凭印象写的。

### ⚠ 下次第一件事（S9c-fix11 之后）

与 S9c 时相同，仍然**只有一条**：**"库文件挤占清单名额"没修**
（那份 CubeMX 工程 40 条里 25 条是 `Drivers/CMSIS/**`）。判据需要用户拍板，写死会误伤别人的工程。

**另外，这一条要用户确认**：S9c 与 fix11 都只过了自动化，实操验收还没做。
装新包后最值得看的：**取件日志里模型写的是不是 `f1` 这样的假名**；
以及**只打开一个文件**（不打开文件夹）时，第 2 轮是不是真的能读到一个别的文件了。

---

## S9c-fix12 去掉假名，清单只给名字（D124）

**目标**：用户看公告时一句反问把假名推翻了："文件名也是有信息的，你改成 f1、f2 什么的是多此一举。"
核实之后发现**我给假名写的头号理由（隐私）本来就是错的**——`describeAnchor` 早就把锚点的
绝对路径与所在目录写进 prompt 了。详见 `DECISIONS.md` D124。

**范围**：

- `relatedFiles.ts`：`CandidateFile` 去掉 `alias`；`buildCandidateFiles` 不再编号；
  `findCandidate` 删掉假名分支（只认名字照抄 + 唯一后缀）；`describeCandidates` 一行一个名字
- `validateContextRequest.ts`：拒绝文案不再提假名，改为"照抄清单里那一行 +
  不要把名字改写成别的形状"
- `Orchestrator.ts`：`candidateModeOf` 的 `'alias'` 改名 `'list'`；`fetchFailureText` 同步
- `prompts/index.ts` + `en.ts`：`fetchSection` 与 user prompt 的清单一节改口径
  （照抄名字；明说"不要改写成别的形状"——那是上一版实测里最贵的动作）
- `toolSchema.ts`：`path` 的 description 同步

**验收标准**：

| 验收 | 靠什么 |
|---|---|
| 清单只给名字、一行一个 | 单测 `describeCandidates`；冒烟断言 prompt 里是 `^- ring_buffer.h$` |
| 写 `f1` 不再被认 | 单测两条：`findCandidate` 不认 `f1`；闸门拒绝写 `f1` 的请求 |
| 名字照抄（含写短了的唯一后缀）仍走得通 | 单测 `findCandidate` + 闸门"照抄清单里那一行"那条 |
| 提示词口径随清单是否为空切换 | 单测两条（空 → 不许说"照抄清单"；非空 → 说"照抄"） |

**回退点**：`slice-S9a-fix11` 之前（S9c-fix11 之后）。假名机制整段删除即回退。

**状态**：**完成**。

### S9c-fix12 落地结果（2026-09-22）

- **单测**：extension-anchor **365 例全过**（+0：改写多于新增，删掉了假名相关的 6 条、新增 2 条口径断言）。
- **冒烟**：链式 212 条断言全过（清单断言从「有 `f1` 那一行」改为「有 `ring_buffer.h` 那一行」，
  "写假名 f1 能取到"改为"照抄名字能取到"）；四组共 393 条；唯一 FAIL 仍是那条 `spawn EBUSY`。
- **typecheck / build / VSIX 四道校验**：全过，`Version` = 0.1.1（第三次重打包）。

### ⚠ 下次第一件事（S9c-fix12 之后）

仍是那一条：**"库文件挤占清单名额"没修**（那份 CubeMX 工程 40 条里 25 条是 `Drivers/CMSIS/**`）。
判据需要用户拍板，写死会误伤别人的工程。

**实操验收还没做**：S9c / fix11 / fix12 三片都只过了自动化。
