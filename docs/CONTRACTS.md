...
| `packages/extension-anchor/src/orchestrator/validateContextRequest.ts` | **S3 落地，S9a 改写规则 3**。§3.2 五条规则的实现 + 跨文件边界（`ContextFetchPolicy`，缺省 `RESTRICTED_POLICY` = 只允许锚点文件） | `FetchScope`:33 `ContextFetchPolicy`:35 `RESTRICTED_POLICY`:45 `FetchedSpan`:78 `ContextFetchState`:99 `ContextDecision`:115 `validateContextRequest`:146 |
...
| `packages/extension-anchor/src/commands.ts` | §4.1 十个命令 + 四层装配 + 捕获确认（§4.1.1）+ 取件日志落 OutputChannel + **S8 的开始面板装配与 `showStart`** + **S9a 的 `fetchPolicyFor` 与第二道闸门**。**S3 起没有任何替身** | `registerCommands`:118 `askWhatToExplain`:711 `capture`:736（S8 新增的 `makeStartModel` / `runStartAction` / `showStart` 在文件后段） |
